...
export default function InstagramFeed({ posts }: InstagramFeedProps) {
    useEffect(() => {
        ...
        // If script is already loaded, just process
        if (window.instgrm) {
            processEmbeds();
        } else {
            // Load script dynamically to avoid hydration mismatch
    ...
    if (!posts || posts.length === 0) {
        return <div className="text-white text-center">No posts found.</div>;
    }

    // Ensure we only take first 3 posts for the feed
        ...
        <div className="w-full">
            <div className="flex flex-wrap justify-center gap-4 items-stretch">
                {displayPosts.map((post) => (
                    <div key={post.id} className="w-full md:w-[calc(50%-1rem)] lg:w-[calc(33.33%-1rem)] flex justify-center bg-white rounded-md overflow-hidden shadow-sm">
                        <blockquote
...
// Add global type definition for window.instgrm
declare global {
    interface Window {
        instgrm?: {
            Embeds: {
